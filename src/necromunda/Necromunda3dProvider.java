...
public class Necromunda3dProvider extends SimpleApplication {
    ...
    public Necromunda3dProvider(Necromunda game) {
        ...
        settings.setSettingsDialogImage(String.format("/Images/Application/Splashscreen%s.png", Utils.rollD(3)));
    ...
    public void simpleInitApp() {
        ...
        switch (Utils.rollD(3)) {
            case 1:
                sky = createSky("undersky_lt.png", "undersky_rt.png", "undersky_ft.png", "undersky_bk.png", "undersky_tp.png", "undersky_bt.png");
                break;
            case 2:
                sky = createSky("SumpFalls_lt.png", "SumpFalls_rt.png", "SumpFalls_ft.png", "SumpFalls_bk.png", "SumpFalls_tp.png", "SumpFalls_bt.png");
                break;
            case 3:
                sky = createSky("SkyLeft.png", "SkyRight.png", "SkyFront.png", "SkyBack.png", "SkyTopBottom.png", "SkyTopBottom.png");
                break;
        }
    ...
    private Node createSmallTableNode() {
        ...
        tableGeometry.setMaterial(materialFactory.createNeoTextureMaterial("Images/Textures/Table/"
                + getTerrainMaterialIdentifier()));
    ...
    private Node createLargeTableNode() {
        Box box = new Box(new Vector3f(100, -0.5f, 100), 100, 0.5f, 100);
        Geometry tableGeometry = new Geometry("tableGeometry", box);
        tableGeometry.setMaterial(materialFactory.createNeoTextureMaterial("Images/Textures/Table/"
                + getTerrainMaterialIdentifier()));
        tableGeometry.setShadowMode(ShadowMode.Receive);
    ...
    private Geometry createSky(String westImageName, String eastImageName, String northImageName, String southImageName, String upImageName, String downImageName) {
        Texture tex_0 = createSkyTexture(upImageName);
        Texture tex_1 = createSkyTexture(westImageName);
        Texture tex_2 = createSkyTexture(eastImageName);
        Texture tex_3 = createSkyTexture(northImageName);
        Texture tex_4 = createSkyTexture(southImageName);
        Texture tex_5 = createSkyTexture(downImageName);

        Geometry sky = (Geometry) SkyFactory.createSky(assetManager, tex_1, tex_2, tex_3, tex_4, tex_0, tex_5);
    ...
    private Texture createSkyTexture(String imageName) {
        TextureKey key = new TextureKey(String.format("Images/Textures/Sky/%s", imageName), true);
    ...
    public void simpleUpdate(float tpf) {
        ...
        if (currentPath != null) {
            ...
            if (!intersect(currentPathNode, otherNodes) && !intersect(selectedFighterNode, otherNodes)) {
                currentPathNode.setMaterial(materialFactory.createTransparentColourMaterial(
                        new ColorRGBA(0.5f, 0.5f, 0.5f, 0.5f), 0.5f));
            }
            else {
    ...
    private Vector3f getHalfExtentsOf(FighterNode fighterNode) {
        ...
        Vector3f halfExtents = new Vector3f(boundingVolume.getRadius(), boundingVolume.getHeight() / 2,
    ...
    private void setBaseNormal(FighterNode fighterNode) {
        Material baseMaterial = materialFactory.createTextureMaterial("Images/Textures/Base/BaseGrass.png", new ColorRGBA(
                64 / 255f, 147 / 255f, 91 / 255f, 1.0f));
        fighterNode.setBaseMaterial(baseMaterial);
    }
    ...
    private void setUpTargeting() {
        ...
        if (line != null) {
            if (currentTemplateNode == null) {
                currentTemplateNode = TemplateNode.createTemplateNode(assetManager, selectedFighterNode,
                        getSelectedRangeCombatWeapon().getCurrentAmmunition());
        ...
        if (selectedFighterNode != null) {
            ...
            if (weapon != null) {
                ...
                if (weapon.getCurrentAmmunition() != null) {
                    ...
                    if (shotHandler instanceof SustainedFireShotHandler) {
                        SustainedFireShotHandler sustainedFireShotHandler = (SustainedFireShotHandler) shotHandler;
                        numberOfSustainedFireDice = "Sustained Fire Dice: "
                                + sustainedFireShotHandler.getNumberOfSustainedFireDice() + ", ";
                    }
                }

                statusText.append(String.format("%s%s%s%s%s\n", weapon, broken, mode, numberOfSustainedFireDice,
                        weapon.getProfileString()));
            }
            else {
